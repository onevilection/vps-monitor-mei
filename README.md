...
- 外観（キャラ表情マップ・背景不透明度）: `%APPDATA%\VpsWatcher\appearance.json`（テンプレートは [`appearance.example.json`](appearance.example.json)）。`background_opacity` が 0.3 未満のときは文字色を自動で濃色に切り替える。
